...
class IPv4UnitTests(unittest.TestCase):
    ...
    def test_ipv4_option_constructors_cover_common_and_error_branches(self) -> None:
        ...
        # The padding terminator is an EOOL *option*, not the bare wire code:
        # these two assertions pinned the latter, which is what #506 fixed.
        self.assertEqual([type(item).__name__ for item in options], ['bytes', 'SIDOption', 'NOPOption', 'EOOLOption'])
        self.assertEqual(options[-1].type, OptionNumber.EOOL)
