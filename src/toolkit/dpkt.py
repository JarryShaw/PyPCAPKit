...
def ipv4_reassembly(packet, *, count=NotImplemented):
    ...
    if ipv4 is not None:
        if ipv4.df:   return False, None            # dismiss not fragmented packet
        data = dict(
            bufid = (
...
def ipv6_reassembly(packet, *, count=NotImplemented):
    ...
    if ipv6 is not None:
        ...
        if ipv6_frag is None:   return False, None  # dismiss not fragmented packet
...
def tcp_traceflow(packet, timestamp, *, data_link, count=NotImplemented):
    ...
    if tcp is not None:
        ...
        data = dict(
            ...
            frame = packet2dict(packet, timestamp), # extracted packet
