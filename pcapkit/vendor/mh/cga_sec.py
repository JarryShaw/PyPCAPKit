...
class CGASec(Vendor):
    ...
    #: Link to registry.
    LINK = 'https://www.iana.org/assignments/cga-message-types/cga-message-types-3.csv'
    ...
    def process(self, data: 'list[str]') -> 'tuple[list[str], list[str]]':
        ...
        for item in reader:
            ...
            try:
                code, _ = item[0], int(item[0], base=2)
                ...
                miss.append(f'    #: {desc}')
                miss.append(f"    return extend_enum(cls, '{self.safe_name(name)}_%s' % bin(value)[2:], value)")

        # TODO: figure out how to handle this programmatically
        miss.append('#: Unspecified in the IANA registry')
        miss.append("return extend_enum(cls, 'Unassigned_%s' % bin(value)[2:], value)")
        return enum, miss


if __name__ == '__main__':
    sys.exit(CGASec())  # type: ignore[arg-type]
