...
class FlowBindingType(Vendor):
    """Flow Binding Type"""

    #: Value limit checker.
    FLAG = 'isinstance(value, int) and 0 <= value <= 255'
    #: Link to registry.
    LINK = 'https://www.iana.org/assignments/mobility-parameters/flow-binding-type.csv'
    ...
    def process(self, data: 'list[str]') -> 'tuple[list[str], list[str]]':
        ...
        for item in reader:
            ...
            try:
                ...
                renm = self.rename(name.replace('Flow Binding', '').strip(),
                                   code, original=long)
