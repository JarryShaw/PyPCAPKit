...
:rfc:`2661` -- a 16-bit tunnel ID and a 16-bit session ID, with the version
nibble reading ``2``. Its structure is described as below:
...
# NOTE: ``schema`` and ``data`` are stated here rather than inherited from
# :class:`~pcapkit.protocols.link.l2tp.L2TP`, which declares none: the base is
# version-agnostic and the header schema is not. They must be passed explicitly
# in any case, since
...
# :mod:`pcapkit.protocols.schema` -- there is no ``L2TPv2`` there -- and assigns
# unconditionally, so omitting them would silently bind ``Schema_Raw``/``Data_Raw``.
class L2TPv2(L2TP[Data_L2TP, Schema_L2TP],
    ...
    @property
    def name(self) -> 'Literal["Layer 2 Tunnelling Protocol version 2"]':
        """Name of current protocol."""
        return 'Layer 2 Tunnelling Protocol version 2'

    @property
    def alias(self) -> 'Literal["L2TPv2"]':
    ...
    def id(cls) -> 'tuple[Literal["L2TP"], Literal["L2TPv2"]]':
        ...
        Returns:
            Index ID of the protocol -- the canonical family name first, then
            this version's own label, as
            :meth:`HTTPv1.id <pcapkit.protocols.application.httpv1.HTTP.id>`
            does. Element zero is what callers treat as canonical, so both
            ``protocol='L2TP'`` and ``protocol='L2TPv2'`` select this class.
    ...
    def __index__(cls) -> 'NoReturn':  # pylint: disable=invalid-index-returned
        ...
        Note:
            ...
            ``L2TPv3`` will be the first class in this family with one, IANA
            protocol number 115.
