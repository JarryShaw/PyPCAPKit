...
&emsp; A tour of `pcapkit` by example: each script here does one thing and prints or dumps the
...
&emsp; Every script reads its capture with a path relative to its own directory, so **run them
...
&emsp; The captures live in `../captures/`. Only `in.pcap` and `dhcp.pcapng` are committed; the
...
&emsp; `_engine_support.py` is not a demo -- it is the shared helper the engine demos use to
work out whether an engine can run on this host.

## What needs more than pcapkit

&emsp; Most of these run with nothing but `pcapkit`. These do not:
...
&emsp; Four files in `../captures/` are committed outputs rather than inputs, and they are
...
&emsp; That is the whole recipe. It runs, equivalently:
...
&emsp; Both inputs are committed, so this works on a fresh clone with no `make samples` first.
Use `make fixtures PYTHON=../../.venv/bin/python` to pick a specific interpreter.

&emsp; **`TZ=UTC` is not decoration.** `pcapkit` renders frame timestamps in the host's local
